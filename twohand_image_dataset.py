...
class ImageDataset(Dataset):
    ...
    def process_webdataset_tar_item(item, train, 
        ...
        key = item['__key__']
        image = item['jpg']
        left_data = item['left_data.pyd']
        left_mask = item['left_mask']
        

        left_keypoints_2d = left_data['keypoints_2d']
        left_keypoints_3d = left_data['keypoints_3d']
        # other_hand_keypoints = data['other_hand_keypoints']
        left_center = left_data['center']
        left_scale = left_data['scale']
        left_hand_pose = left_data['hand_pose']
        left_betas = left_data['betas']
        # right = data['right']
        # print(f'right:{right}')
        left_has_hand_pose = left_data['has_hand_pose']
        left_has_betas = left_data['has_betas']
        # other_hand_center = data.get('other_hand_center', None)
        # other_hand_scale = data.get('other_hand_scale', None)
        # Process bounding boxes

        # Single hand case
        left_box_size = left_scale * 200  # scale is typically normalized, so multiply by 200 to get pixel size
        x_min = int(left_center[0] - left_box_size[0] / 2)
        x_max = int(left_center[0] + left_box_size[0] / 2)
        y_min = int(left_center[1] - left_box_size[1] / 2)
        y_max = int(left_center[1] + left_box_size[1] / 2)
        ...
        # Process data
        left_orig_keypoints_2d = left_keypoints_2d.copy()
        left_center_x = left_center[0]
        left_center_y = left_center[1]
        left_bbox_size = expand_to_aspect_ratio(left_scale*200, target_aspect_ratio=BBOX_SHAPE).max()

        left_mano_params = {'global_orient': left_hand_pose[:3],
                    'hand_pose': left_hand_pose[3:],
                    'betas': left_betas
                    }

        left_has_mano_params = {'global_orient': left_has_hand_pose,
                        'hand_pose': left_has_hand_pose,
                        'betas': left_has_betas
                        }

        left_mano_params_is_axis_angle = {'global_orient': True,
                                    'hand_pose': True,
                                    'betas': False
                                    }
        ...
        # Crop image and (possibly) perform data augmentation
        left_img_rgba = np.concatenate([image, left_mask.astype(np.uint8)[:,:,None]*255], axis=2)
        # img_rgba = image.copy()
        right = 0
        left_img_patch_rgba, left_keypoints_2d, left_keypoints_3d, left_mano_params, left_has_mano_params, img_size, left_trans = get_example(left_img_rgba,
                                                                                                        left_center_x, left_center_y,
                                                                                                        left_bbox_size, left_bbox_size,
                                                                                                        left_keypoints_2d, left_keypoints_3d,
                                                                                                        left_mano_params, left_has_mano_params,
        ...
        left_img_patch = left_img_patch_rgba[:3,:,:]
        left_mask_patch = (left_img_patch_rgba[3,:,:] / 255.0).clip(0,1)
        if (left_mask_patch < 0.5).all():
            left_mask_patch = np.ones_like(left_mask_patch)

        right_data = item['right_data.pyd']
        right_mask = item['right_mask']
        

        right_keypoints_2d = right_data['keypoints_2d']
        right_keypoints_3d = right_data['keypoints_3d']
        # other_hand_keypoints = data['other_hand_keypoints']
        right_center = right_data['center']
        right_scale = right_data['scale']
        right_hand_pose = right_data['hand_pose']
        right_betas = right_data['betas']
        # right = data['right']
        # print(f'right:{right}')
        right_has_hand_pose = right_data['has_hand_pose']
        right_has_betas = right_data['has_betas']
        # other_hand_center = data.get('other_hand_center', None)
        # other_hand_scale = data.get('other_hand_scale', None)
        # Process bounding boxes

        # Single hand case
        right_box_size = right_scale * 200  # scale is typically normalized, so multiply by 200 to get pixel size
        x_min = int(right_center[0] - right_box_size[0] / 2)
        x_max = int(right_center[0] + right_box_size[0] / 2)
        y_min = int(right_center[1] - right_box_size[1] / 2)
        y_max = int(right_center[1] + right_box_size[1] / 2)
        ...
        # Process data
        right_orig_keypoints_2d = right_keypoints_2d.copy()
        right_center_x = right_center[0]
        right_center_y = right_center[1]
        right_bbox_size = expand_to_aspect_ratio(right_scale*200, target_aspect_ratio=BBOX_SHAPE).max()

        right_mano_params = {'global_orient': right_hand_pose[:3],
                    'hand_pose': right_hand_pose[3:],
                    'betas': right_betas
                    }

        right_has_mano_params = {'global_orient': right_has_hand_pose,
                        'hand_pose': right_has_hand_pose,
                        'betas': right_has_betas
                        }

        right_mano_params_is_axis_angle = {'global_orient': True,
                                    'hand_pose': True,
                                    'betas': False
                                    }
        ...
        augm_config = copy.deepcopy(augm_config)
        # Crop image and (possibly) perform data augmentation
        right_img_rgba = np.concatenate([image, right_mask.astype(np.uint8)[:,:,None]*255], axis=2)
        # img_rgba = image.copy()
        right = 1
        right_img_patch_rgba, right_keypoints_2d, right_keypoints_3d, right_mano_params, right_has_mano_params, img_size, right_trans = get_example(right_img_rgba,
                                                                                                        right_center_x, right_center_y,
                                                                                                        right_bbox_size, right_bbox_size,
                                                                                                        right_keypoints_2d, right_keypoints_3d,
                                                                                                        right_mano_params, right_has_mano_params,
                                                                                                        FLIP_KEYPOINT_PERMUTATION,
                                                                                                        IMG_SIZE, IMG_SIZE,
                                                                                                        MEAN, STD, train, right, augm_config,
                                                                                                        is_bgr=False, return_trans=True,
        ...
        right_img_patch = right_img_patch_rgba[:3,:,:]
        right_mask_patch = (right_img_patch_rgba[3,:,:] / 255.0).clip(0,1)
        if (right_mask_patch < 0.5).all():
            right_mask_patch = np.ones_like(right_mask_patch)
        # 生成并存储单手数据的item
        item = {}
        # 判断是左手还是右手
        # hand_type = 'right' if right > 0.5 else 'left'

        item['left_img'] = left_img_patch
        item['left_mask'] = left_mask_patch
        item['left_keypoints_3d'] = left_keypoints_3d.astype(np.float32)
        item['left_orig_keypoints_2d'] = left_orig_keypoints_2d
        item['left_box_center'] = left_center.copy()
        item['left_box_size'] = left_bbox_size
        item['left_bbox'] = left_bbox
        item['left_mano_params'] = left_mano_params
        item['left_has_mano_params'] = left_has_mano_params
        item['left_mano_params_is_axis_angle'] = left_mano_params_is_axis_angle
        item['left__scale'] = left_scale
        item['left__trans'] = left_trans
        item['right_img'] = right_img_patch
        item['right_mask'] = right_mask_patch
        item['right_keypoints_3d'] = right_keypoints_3d.astype(np.float32)
        item['right_orig_keypoints_2d'] = right_orig_keypoints_2d
        item['right_box_center'] = right_center.copy()
        item['right_box_size'] = right_bbox_size
        item['right_bbox'] = right_bbox
        item['right_mano_params'] = right_mano_params
        item['right_has_mano_params'] = right_has_mano_params
        item['right_mano_params_is_axis_angle'] = right_mano_params_is_axis_angle
        item['right__scale'] = right_scale
        item['right__trans'] = right_trans       
        item['imgname'] = key
        item['img_size'] = 1.0 * img_size[::-1].copy()
